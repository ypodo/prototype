...
begin
  ...
  #puts Dir.pwd
  path_to_db="/home/ubuntu/Documents/prototype/db/development.sqlite3"
  puts "invite id: "+invite_id
  puts "User id: "+user_id
  puts "Value to update: "+value_to_update
  update_str='UPDATE invites SET arriving='+value_to_update+"  WHERE id="+invite_id
  puts update_str
  #sqlite3 db/development.sqlite3 "select name from invites where id=144"
  db = SQLite3::Database.open path_to_db
  name = db.execute "SELECT name from invites where id="+invite_id
  if !name.nil?
    puts name
